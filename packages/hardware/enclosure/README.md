...
- EC600X-EVB 按用户提供的《EC600X系列开发板丝印.pdf》建立约 `85 × 46 × 1.6 mm` 的非打印参考包络，板框放在前壳底盘 `X=33..118 mm`、`Y=4.5..50.5 mm`，按图中 2.54 mm 排针间距换算；丝印图中 QuecPython 旁的单个红圈作为唯一 M2 安装孔参考。板旋转 180° 放置，USB-C 朝 +X 的按键侧。
- EC600X 固定结构位于前壳底盘：四个墙厚 `1.5 mm`、与板边单边间隙 `0.4 mm` 的 L 型限位，限位顶部高出板顶 `0.4 mm`；一个外六角 AF `7.5 mm` 的 M2 基座，内置 AF `5.0 mm × 5.0 mm` 六角热熔嵌件槽和 `2.4 mm` 贯穿孔。基座和限位从 `Z=3.0 mm` 起与底盘融合，适配实际打印。
...
6. EC600X 安装在前壳底盘的四个 L 型限位内，先将 QuecPython 侧的单个 M2 孔对准六角基座，再压入一颗 M2 六角铜柱或使用 M2 螺钉固定；USB-C 朝按键侧。安装前确认 USB-C、天线和排针不会压到屏幕后方结构。
