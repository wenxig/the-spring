...
这是参照用户提供的圆角矩形设备照片制作的两件式外壳。顶部保持完整平面；前壳正面左侧为电子纸显示窗，右下保留按键开窗和中间隔板，后盖可拆下以放入元件，并作为 ESP32 开发平台。
...
- ESP32-P4-Module-DEV-KIT 按 `85 × 56 × 1.6 mm` 的参考包络放在后盖内侧，X 范围为 `10..95 mm`、Z 范围为 `10..66 mm`，大部分位于左侧显示区域的后方投影。开发板的 GPIO/排针面朝向前壳和屏幕侧；USB、网口、PoE 等接口均保留在壳体内部，未开外露接口孔。
- 开发板安装孔中心距按图片采用 `58 × 49 mm`，四个 M2 黄铜热熔嵌件座位于后盖内侧，座体直径 7 mm、嵌件压入孔直径 3.0 mm、深度 4.0 mm。后盖安装开发板后，前壳从前方扣入并通过另一组四颗 M2 螺钉固定。
...
- 后盖开发板座使用同一组 M2 嵌件参数，开发板孔位置可通过 `DevboardX`、`DevboardZ` 和两个孔距参数调整。
