...
- EC600X-EVB 采用约 `88 × 48 × 1.6 mm` 参考板，长边沿左右方向立放、板面平行屏幕，元件面朝后盖，USB-C 朝右侧按键方向。坐标为 `X=28..116 mm`、`Y=20..21.6 mm`、`Z=8..56 mm`。板框根据用户 PDF 的 2.54 mm 排针间距换算，厚度和孔径为假设值，需实物确认。
...
- EC600X 裸板与 ESP32 裸板的前后净距为 `27.4 mm`，USB-C 板边到右侧内壁约 `10 mm`。这些数值须结合器件高度、插头和线缆弯折核对。板背面到前侧内壁平面为 `16 mm`；屏幕实体、按键板、插头与实际焊点尚未纳入几何包络。
