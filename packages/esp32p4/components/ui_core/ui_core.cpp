...
void Frame::text(std::uint16_t x, std::uint16_t y, const char* value) { for (std::size_t i = 0; value[i] != '\0'; ++i) { const auto ox = static_cast<std::uint16_t>(x + i * 6); const auto code = static_cast<unsigned char>(value[i]); for (auto dy = 0; dy < 7; ++dy) for (auto dx = 0; dx < 5; ++dx) { const auto bit = static_cast<unsigned char>((code * 13U + dy * 7U + dx * 3U) >> (dx % 5)); if ((bit & 1U) != 0U) pixel(ox + dx, y + dy); } } }
