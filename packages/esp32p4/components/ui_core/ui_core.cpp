...
Rect align_dirty(Rect r) { if (r.width == 0 || r.height == 0) return {}; const auto left = static_cast<std::uint16_t>(r.x & ~7U); const auto right = static_cast<std::uint16_t>((r.x + r.width + 7U) & ~7U); return {left, r.y, static_cast<std::uint16_t>(right - left), r.height}; }
