...
if _type_checking:
    from type_contracts import CallbackArgs, CallCallback, Platform
...
class VolteService:
    def __init__(
        ...
        event_cb: "CallCallback | None" = None,
    ) -> None:
        self._voice_call = platform.voice_call
        self._auto_answer = auto_answer
        self._event_cb = event_cb
        self._current_call_id: "object | None" = None
    ...
    def _register_callback(self) -> None:
        def _cb(args: "CallbackArgs") -> None:
            ...
            phone_value = args[6] if len(args) > 6 else ""
            phone = phone_value if isinstance(phone_value, str) else str(phone_value)
    ...
    def set_channel(self, channel: int = 0) -> None:
        """切换音频通道 (0: 听筒/喇叭, 1: 耳机)"""
        try:
