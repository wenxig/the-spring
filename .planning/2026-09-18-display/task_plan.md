...
2. 实现共享 XML 组件渲染、运行时 TTF 字体、Ionicons 图标与天气列布局。Status: in_progress
3. 修复日期有效性、6月7日至9日状态及天气数据语义，增加边界验证。Status: pending
4. 仿真视觉验证、静态检查、固件构建、烧录和日志核验。Status: pending
5. 板载 ESP32-C6 Wi-Fi 优先链路、蜂窝回退与真实板验证。Status: in_progress

## Next Step
烧录 `/dev/cu.usbmodem141101`，读取启动日志并确认 C6 握手、网络回退和电子纸画面。
